...
Follow these steps to set up your environment for the robot simulation and navigation stack (

### 1. Set Locale (if not already set)
```bash
...
### 3. Install ROS 2 Humble Desktop (includes Gazebo, RViz, and most tools)
```bash
sudo apt update
sudo apt upgrade
...
### 5. Install Development Tools (for building packages)
```bash
...
After building, source your workspace (replace <your_ws> with your workspace directory):
```bash
...
# Workspace Installation and Folder Structure

This project is designed for **ROS 2 Humble** on **Ubuntu 22.04** (including WSL). The main workspace is located in the `ROS2_4wheel_autonomous_robot` directory. Below are the steps to set up, build, and understand the folder structure of the workspace.

## 1. Clone or Copy the Workspace

Place the `ROS2_4wheel_autonomous_robot` folder in your desired location (e.g., `~/` or `~/ros2_ws/`).

## 2. Install System and ROS 2 Dependencies

Follow the [Installation and Dependencies](#installation-and-dependencies) section below to install all required packages for ROS 2 Humble, Gazebo, RViz, and development tools.

## 3. Folder Structure Overview

- `ROS2_4wheel_autonomous_robot/`
  - `src/` — Contains all ROS 2 packages for simulation, navigation, and SLAM:
    - `robot_description/` — Robot model, URDF/Xacro, launch files, and Gazebo/RViz configs.
    - `robot_nav/` — Navigation stack (Nav2) configuration and launch files.
    - `robot_slam/` — SLAM (Simultaneous Localization and Mapping) configuration and launch files.
  - `build.sh` — Helper script to build the workspace and convert Xacro to URDF.
...
```bash
cd ~/ROS2_4wheel_autonomous_robot
...
- The `build.sh` script will:
  ...
  - Source the workspace setup file for your shell.
  - Convert the main Xacro file to URDF for the robot model.
  - Rebuild to ensure the URDF is included.
...
After building, always source the setup file in every new terminal:

```bash
source /opt/ros/humble/setup.bash
source ~/ROS2_4wheel_autonomous_robot/install/setup.bash
```

## 6. Folder-Specific Usage and Commands

- **robot_description/**
  - Contains robot model files (`models/myCar/BMW.urdf.xacro`), launch files, and configs for simulation.
  - To convert Xacro to URDF manually:
    ```bash
    xacro src/robot_description/models/myCar/BMW.urdf.xacro > src/robot_description/models/myCar/BMW.urdf
    ```
...
- **robot_slam/**
  - Contains SLAM launch and config files.
  - To start mapping:
    ```bash
...
- **robot_nav/**
  - Contains navigation stack (Nav2) launch and config files.
  - To start navigation:
    ```bash
...
## 7. Notes
- Always build the workspace after making changes to any package.
- Always source the setup file in every new terminal before running ROS 2 commands.
- For more details, see the rest of this README and the main workspace README.
...
# Getting Started afteer following the above steps to the end
## Mapping
For mapping the environment launch the following launch files each in a separate terminal:
```bash
 
 # Launch rviz, gazebo and robot state and joint publisher
ros2 launch robot_description launch_sim_ros_control.launch.py world:=./src/robot_description/models/myWorld/boxes_world.sdf use_sim_time:=true rviz_config_file:=./src/robot_description/rviz/sim.config.rviz
